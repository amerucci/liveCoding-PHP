    ...
    if (this.readyState == 4 && this.status == 200) {
      ...
      for (let i = 0; i < communess.length; i++) {
        ...
        laliste.innerHTML +=
          ...
          communess[i][0] +
          '</th><th scope="col">' +
          communess[i].cp_commune +
          '</th><th scope="col">' +
          communess[i].population_commune +
          '</th><th scope="col">' +
          communess[i].nom_departement +
          '</th><th scope="col">' +
          communess[i].nom_region +
          "</th></tr>";
      }
    }
  };
  ...
  if (region.value != "") {
    ...
    xmlhttp.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        ...
        document.getElementById("lesdepartements").innerHTML +=
          '<option value="">Selecionnez une région</option>';
        for (let i = 0; i < departements.length; i++) {
          document.getElementById("lesdepartements").innerHTML +=
            '<option value="' +
            departements[i].nom_departement +
            '">' +
            departements[i].nom_departement +
            "</option>";
        }
    ...
    if (this.readyState == 4 && this.status == 200) {
      ...
      for (let i = 0; i < communes.length; i++) {
        ...
        laliste.innerHTML +=
          ...
          communes[i][0] +
          '</th><th scope="col">' +
          communes[i].cp_commune +
          '</th><th scope="col">' +
          communes[i].population_commune +
          '</th><th scope="col">' +
          communes[i].nom_departement +
          '</th><th scope="col">' +
          communes[i].nom_region +
          "</th></tr>";
      }
    }
  };
